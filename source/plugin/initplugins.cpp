...
template<class T>
T convolveGrid(const Grid<T>& originGrid, GaussianKernelCreator& gkSigma, Vec3 pos, int cdir){
...
void knBlurGrid(const Grid<T>& originGrid, Grid<T>& targetGrid, GaussianKernelCreator& gkSigma, int cdir){
	targetGrid(i, j, k) = convolveGrid<T>(originGrid, gkSigma, Vec3(i, j, k), cdir);
}

template<class T>
int blurGrid(const Grid<T>& originGrid, Grid<T>& targetGrid, float sigma){
...
PYTHON() int blurRealGrid(const Grid<Real>& oG, Grid<Real>& tG, float si){
