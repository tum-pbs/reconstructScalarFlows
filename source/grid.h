...
public:
	enum GridType { TypeNone = 0, TypeReal = 1, TypeInt = 2, TypeVec3 = 4, TypeVec3i = 5, TypeMAC = 8, TypeLevelset = 16, TypeFlags = 32 };
...
public:
	...
	//! get mean value in grid 
	PYTHON() Real getMean() const;
	//! get unscaled mean value in grid 
	PYTHON() Real getMeanUnscaled() const;
	//! get number of non-zero value cells in grid 
	PYTHON() Real getCellCount() const;
	//! get standard deviation in grid 
	PYTHON() Real getStdDeviationUnscaled(const Real mean) const;
...
PYTHON() alias Grid<Vec3i> VeciGrid;
...
public:
	...
	inline bool isSrc(IndexInt idx) const { return get(idx) & TypeSrc; }
	inline bool isSrc(int i, int j, int k) const { return get(i, j, k) & TypeSrc; }
	inline bool isSrc(const Vec3i& pos) const { return get(pos) & TypeSrc; }
	inline bool isSrc(const Vec3& pos) const { return getAt(pos) & TypeSrc; }
