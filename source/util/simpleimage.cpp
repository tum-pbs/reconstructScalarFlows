...
{	
	...
	default: {
		// volumetrics / smoke
		if (std::fabs(src)>1e-12) {
			if (dst.x>dst.y && src>0) { // if negative destination and positive source 
				if (dst.x >= depthInv*src) dst += Vec3(-depthInv*src, 0, 0);
				else dst = Vec3(depthInv*src - dst.x, depthInv*src - dst.x, depthInv*src - dst.x);
			}
			else if (dst.x == dst.y && dst.x>0 && src<0) { // if positive destination and negative source 
				if (dst.x >= depthInv*std::fabs(src)) dst += depthInv * Vec3(src, src, src);
				else dst = Vec3(std::fabs(depthInv*src - dst.x), 0, 0);
			}
			else {
				if (src<0) dst += depthInv * Vec3(std::fabs(src), 0, 0);
				else dst += depthInv * Vec3(src, src, src);
			}
		}
	} break;
