...
template<> Vec3i* FluidSolver::getGridPointer<Vec3i>() {
	return mGridsVeci.get(mGridSize);
}
...
template<> void FluidSolver::freeGridPointer<Vec3i>(Vec3i* ptr) {
	mGridsVeci.release(ptr);
}
...
void FluidSolver::printMemInfo() {
	...
	msg << "                 vec3i " << mGridsVeci.used << "/" << mGridsVeci.grids.size() << ". ";
