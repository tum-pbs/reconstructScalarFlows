...
template<> inline GridBase::GridType typeList<Vec3i>() { return GridBase::TypeVec3i; }
...
//! Kernel: Compute mean value of Real grid
KERNEL(idx, reduce = +) returns(Real sum = 0)
Real CompMeanReal(const Grid<Real>& val) {
	//if (fabs(val[idx])>1e-9) 
	sum += val[idx];
}

//! Kernel: Compute mean value of Real grid
KERNEL(idx, reduce = +) returns(Real count = 0)
Real CompCountReal(const Grid<Real>& val) {
	if (fabs(val[idx]) > 1e-9) count++;
}

//! Kernel: Compute std deviation value of Real grid
KERNEL(idx, reduce = +) returns(Real sum = 0)
Real CompStdDevReal(const Grid<Real>& val, const Real mean) {
	//if (fabs(val[idx])>1e-9) 
	sum += (val[idx] - mean)*(val[idx] - mean);
}
...
//! Kernel: Compute min norm of vec grid
KERNEL(idx, reduce = +) returns(Real sum = 0)
Real CompMeanVec(const Grid<Vec3>& val) {
	const Real s = sqrt(normSquare(val[idx]));
	//if (fabs(s)>1e-6) 
	sum += s;
}
...
//! Kernel: Compute mean value of Real grid
KERNEL(idx, reduce = +) returns(Real count = 0)
Real CompCountVec(const Grid<Vec3>& val) {
	const Real s = sqrt(normSquare(val[idx]));
	if (fabs(s) > 1e-6) count++;
}

//! Kernel: Compute min norm of vec grid
KERNEL(idx, reduce = +) returns(Real sum = 0)
Real CompStdDevVec(const Grid<Vec3>& val, const Real mean) {
	const Real s = sqrt(normSquare(val[idx]));
	//if(fabs(s)>1e-6) 
	sum += (s - mean)*(s - mean);
}
...
//! Kernel: Compute min norm of veci grid
KERNEL(idx, reduce = min) returns(Real minVal = std::numeric_limits<Real>::max())
Real CompMinVeci(const Grid<Vec3i>& val) {
	const Real s = normSquare(val[idx]);
	if (s < minVal)
		minVal = s;
}

//! Kernel: Compute max norm of veci grid
KERNEL(idx, reduce = max) returns(Real maxVal = -std::numeric_limits<Real>::max())
Real CompMaxVeci(const Grid<Vec3i>& val) {
	const Real s = normSquare(val[idx]);
	if (s > maxVal)
		maxVal = s;
}
...
KERNEL(idx) void knGridSafeDivVec3i(Grid<Vec3i>& me, const Grid<Vec3i>& other) {
	for (int c = 0; c < 3; c++) {
		if (other(idx)[c]) me(idx)[c] = me(idx)[c] / other(idx)[c];
	}
}
...
KERNEL(idx) void knGridClampVec3i(Grid<Vec3i>& me, const int& min, const int& max) { 
	for (int c = 0; c < 3; c++) {
		if (me[idx][c] < min) me[idx][c] = min;
		if (me[idx][c] > max) me[idx][c] = max;
	}
}
...
template<> inline void stomp<Vec3i>(Vec3i &v, const Vec3i &th) { if (v[0]<th[0]) v[0] = 0; if (v[1]<th[1]) v[1] = 0; if (v[2]<th[2]) v[2] = 0; }
...
template<> Grid<Vec3i>& Grid<Vec3i>::safeDivide(const Grid<Vec3i>& a) {
	knGridSafeDivVec3i(*this, a);
	return *this;
}
...
template<> void Grid<Vec3i>::clamp(Real min, Real max) {
	knGridClampVec3i(*this, ceil(min), floor(max));
}
...
template<> Real Grid<Real>::getMean() const {
	return (CompMeanReal(*this))/(mSize.x*mSize.y*mSize.z);
}
template<> Real Grid<Real>::getMeanUnscaled() const {
	return (CompMeanReal(*this));
}
template<> Real Grid<Real>::getCellCount() const {
	return (CompCountReal(*this));
}
template<> Real Grid<Real>::getStdDeviationUnscaled(const Real mean) const {
	return CompStdDevReal(*this, mean);
}
...
template<> Real Grid<Vec3>::getMean() const {
	return (CompMeanVec(*this)) / (mSize.x*mSize.y*mSize.z);
}
template<> Real Grid<Vec3>::getMeanUnscaled() const {
	return (CompMeanVec(*this));
}
template<> Real Grid<Vec3>::getCellCount() const {
	return (CompCountVec(*this));
}
template<> Real Grid<Vec3>::getStdDeviationUnscaled(const Real mean) const {
	return (CompStdDevVec(*this, mean));
}
template<> Real Grid<Vec3>::getMaxAbs() const {
	return sqrt(CompMaxVec (*this));
}
template<> Real Grid<Vec3i>::getMax() const {
	return sqrt(CompMaxVeci(*this));
}
template<> Real Grid<Vec3i>::getMin() const {
	return sqrt(CompMinVeci(*this));
}
template<> Real Grid<Vec3i>::getMean() const {
	errMsg("getMean() not implemented for Vec3i");
	return 0;
}
template<> Real Grid<Vec3i>::getMeanUnscaled() const {
	errMsg("getMeanUnscaled() not implemented for Vec3i");
	return 0;
}
template<> Real Grid<Vec3i>::getCellCount() const {
	errMsg("getCellCount() not implemented for Vec3i");
	return 0;
}
template<> Real Grid<Vec3i>::getStdDeviationUnscaled(const Real mean) const {
	errMsg("getStdDeviationUnscaled() not implemented for Vec3i");
	return 0;
}
template<> Real Grid<Vec3i>::getMaxAbs() const {
	return sqrt(CompMaxVeci(*this));
}
...
template<> Real Grid<int>::getMean() const {
	errMsg("getMean() not implemented for int");
	return 0;
}
template<> Real Grid<int>::getMeanUnscaled() const {
	errMsg("getMeanUnscaled() not implemented for int");
	return 0;
}
template<> Real Grid<int>::getCellCount() const {
	errMsg("getCellCount() not implemented for int");
	return 0;
}
template<> Real Grid<int>::getStdDeviationUnscaled(const Real mean) const {
	errMsg("getStdDeviationUnscaled() not implemented for int");
	return 0;
}
...
PYTHON() Real gridMaxDiffVec3i(Grid<Vec3i>& g1, Grid<Vec3i>& g2)
{
	double maxVal = 0.;
	FOR_IJK(g1) {
		// accumulate differences with double precision
