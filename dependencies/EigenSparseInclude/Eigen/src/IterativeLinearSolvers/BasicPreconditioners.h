...
class LeastSquareDiagonalPreconditioner : public DiagonalPreconditioner<_Scalar>
{
  ...
  public:
    ...
    LeastSquareDiagonalPreconditioner& factorize(const MatType& mat)
    {
	  ...
	  if (m_regLSCG) {
	    for (int j = 0; j<m_regLSCG->outerSize(); ++j)
		  for (SparseMatrix<Real>::InnerIterator it(*m_regLSCG, j); it; ++it) {
			if(it.row()==it.col()) m_invdiag(j) += RealScalar(1) / numext::real(it.value());
	      }
		}
	  }
